...
%import "Commons.mo"
%outer SimInfoManager (replaceable Meteo.location city, replaceable MeteoOnFile detail);
%\end{lstlisting}

determined by \modelica{city} depicting the location of the used climate determined by its latitude and longitude as well as design climate data for system design, and \modelica{detail} depicting the time resolution of the used data.
...
wherefore $r_{D}(t,x)$ and $r_{d}(t,x)$ are the ratios between direct and diffuse radiation respectively on a tilted surface to that on a horizontal surface, defined as

\begin{equation}
r_{D}(t,x)=\cos \xi(t,x)\cos^{-1} \xi_{h}(t)
\end{equation}

\begin{dmath}
r_{d}(t,x)=r_D(t,x) \frac{E_{D,h}(t)}{E_{oh}(t)}+\Omega(t)\cos i(x)+F_{d}(t,x)\left[1 - \frac{E_{D,h}(t)}{E_{oh}(t)}-\Omega(t)\right]
\end{dmath}

and wherefore $\xi(t,x)$ and $\xi_{h}(t)$ are the zenith angle for surface s and the horizontal surface respectively, i.e. the angle between the straight line joining the centers of the earth and the sun and the normal of surface s, $\rho$ is the albedo, i.e. the average reflectivity of the environment, $E_{oh}(t)$ is the total extraterrestrial solar irradiation, $i(x)$ is the surface inclination defined as $0$ for flat roofs and $\pi/2$ for vertical walls, $\Omega(t)$ equals $\max\left\{0,0.3-2 E_{D,h}(t)/E_{oh}(t)\right\}$, $F_{r}(t,x)$ is the isotropic reflected view factor defined as $\left[1-\cos \xi(t,x)\right]/2$ and $F_{d}(t,x)$ is the isotropic diffuse view factor defined as $\left[1+\cos \xi(t,x)\right]/2$.

For calculation of the reflected solar irradiation $E_{r}(t,x)$, the albedo $\rho(t)$ is determined~\cite{Meteotest2010} as

\begin{equation}
\rho(t)=11.895 - 0.042\ T_{db}(t), \forall \rho(t) \in\left[ 0.2,0.8 \right]
\end{equation}

where $T_{db}(t)$ is the average outdoor dry-bulb temperature of the last two weeks.

The total extraterrestrial solar irradiation $E_{oh}(t)$ is defined as~\cite{Duffie1980}

\begin{equation}
E_{oh}(t)=S_{sol}+0.033412\ S_{sol} \cos d(t) 
\end{equation}

where $d(t)$ is day angle and $S_{sol}$ is the solar constant~\cite{Darula2005} equaling 1366.1 \watt\metre\rpsquared.

\biblstarthook{References may be \textit{cited} in the text either by number (preferred) or by author/year.\footnote{Make sure that all references from the list are cited in the text. Those not cited should be moved to a separate \textit{Further Reading} section or chapter.} The reference list should ideally be \textit{sorted} in alphabetical order -- even if reference numbers are used for the their citation in the text. If there are several works by the same author, the following order should be used}

{\bibliography{Buildings/MyBibTexLibrary.bib}{}}
