...
\begin{dmath}
E_{d}(t,x) + E_{r}(t,x) = A E_{d,h} \left[ 0.5 \left(1+\cos(i)\right) \left(1-F_{1}\right) + F_{1} \cos\xi(t,x) \cos^{-1}\xi_{h}(t) + F_{2} \sin(i)\right] + 0.5 \rho A \left(E_{D,h} + E_{d,h}\right) \left(1 - \cos(i)\right)
\end{dmath}

wherefore $A$ is the surface area, $E_{d,h}$ is the diffuse horizonal radiation, $i$ is the surface inclination, $F_{1}$ is the circomsolar brightening coefficient, $F_{2}$, $\rho$ is the ground reflectance and $E_{D,h}$ is the direct horizonal radiation. The brightening coefficients $F_{1}$, $F_{2}$ are determined as
...
\begin{dmath}
\epsilon = \left(E_{d,h} + E_{D,h}\right) E_{d,h}^{-1} + \kappa \xi_{h}^{3}(t) \left(1 + \kappa \xi_{h}^{3}(t)\right)^{-1}
\end{dmath}
