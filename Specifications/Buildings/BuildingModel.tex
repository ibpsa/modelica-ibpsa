...
\runinhead{Convection.} The exterior convective heat flow $\phi_{c}(t,x)$ is computed as
...
\runinhead{Longwave radiation.} Longwave radiation between the surface and environment $\phi_{LW,e}(x)$ is determined as 
...
\runinhead{Shortwave radiation.} Shortwave solar irradiation absorbed by the exterior surface $\phi_{SW}(t,x)$ is determined as $\epsilon_{SW}(x) A(x) E_{S}(t,x)$ where $\epsilon_{SW}(x)$ is the shortwave absorption of the surface, $A(x)$ the surface area and $E_{S}(t,x)$ the total irradiation on the depicted surface. The calculation method for solar irradiation $E_{S}(t,x)$ depending on latitude, time, weather conditions, inclination and orientation is described in detail on page \pageref{chap:climsol}. 
