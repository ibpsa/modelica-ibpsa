within Annex60.Media.Interfaces;
partial package PartialSimpleIdealGasMedium
  ...
  // FluidConstants need to be extended as it is replaceable
  // in the base class. Without this statement, a warning
  // is issued during model check of packages that extend from this package
  ...
  algorithm
    s := cp_const*Modelica.Math.log(state.T/T0)
         - R_gas*Modelica.Math.log(state.p/reference_p);
  end specificEntropy;
