...
required from medium model \""     + mediumName + "\".");
  ...
  /*
    assert(Xi[Water] <= X_sat, "The medium model '" + mediumName + "' must not be saturated.\n"
     + "To model a saturated medium, use 'Annex60.Media.GasesPTDecoupled.MoistAir' instead of this medium.\n"
     + " T         = " + String(T) + "\n"
     + " X_sat     = " + String(X_sat) + "\n"
...
algorithm
  ...
  annotation(Inline=false,smoothOrder=5,derivative=Annex60.Media.GasesPTDecoupled.MoistAirUnsaturated.saturationPressureLiquid_der,
...
algorithm
  h := (T-273.15) * steam.cp + Annex60.Media.GasesPTDecoupled.MoistAirUnsaturated.enthalpyOfVaporization(T);
...
algorithm
  h := Annex60.Media.GasesPTDecoupled.MoistAirUnsaturated.enthalpyOfCondensingGas(T)*X[Water]
       + Annex60.Media.GasesPTDecoupled.MoistAirUnsaturated.enthalpyOfDryAir(T)*(1.0-X[Water]);
end enthalpyOfGas;
...
algorithm
  h := Annex60.Media.GasesPTDecoupled.MoistAirUnsaturated.h_pTX(state.p, state.T, state.X);
end specificEnthalpy;

redeclare function extends specificInternalEnergy "Specific internal energy"
  extends Modelica.Icons.Function;
algorithm
  u := Annex60.Media.GasesPTDecoupled.MoistAirUnsaturated.h_pTX(state.p,state.T,state.X) - pStp/dStp;
end specificInternalEnergy;

redeclare function extends specificGibbsEnergy "Specific Gibbs energy"
  extends Modelica.Icons.Function;
algorithm
  g := Annex60.Media.GasesPTDecoupled.MoistAirUnsaturated.h_pTX(state.p,state.T,state.X) - state.T*specificEntropy(state);
end specificGibbsEnergy;

redeclare function extends specificHelmholtzEnergy "Specific Helmholtz energy"
  extends Modelica.Icons.Function;
algorithm
  f := Annex60.Media.GasesPTDecoupled.MoistAirUnsaturated.h_pTX(state.p,state.T,state.X)
         - gasConstant(state)*state.T
         - state.T*Annex60.Media.GasesPTDecoupled.MoistAirUnsaturated.specificEntropy(state);
...
<a href=\"modelica://Annex60.Media.GasesPTDecoupled.MoistAir\">
Annex60.Media.GasesPTDecoupled.MoistAir</a> instead of this one.
</p>
<p>
