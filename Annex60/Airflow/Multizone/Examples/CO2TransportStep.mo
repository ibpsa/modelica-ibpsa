...
model CO2TransportStep "Model that transport CO2 through buoyancy driven flow"
  extends Annex60.Airflow.Multizone.Validation.ThreeRoomsContam(
    volWes(nPorts=5),
    volTop(nPorts=3),
    volEas(nPorts=6));

  Annex60.Fluid.Sensors.TraceSubstances CO2SenTop(redeclare package Medium = Medium)
    "CO2 sensor"
    annotation (Placement(transformation(extent={{20,120},{40,140}})));
  Annex60.Fluid.Sensors.TraceSubstances CO2SenWes(redeclare package Medium = Medium)
    "CO2 sensor"
    annotation (Placement(transformation(extent={{-102,10},{-82,30}})));
  Annex60.Fluid.Sensors.TraceSubstances CO2SenEas(redeclare package Medium = Medium)
    "CO2 sensor"
  ...
  Annex60.Fluid.Sources.TraceSubstancesFlowSource sou(
...
equation
  ...
  annotation (
    Diagram(coordinateSystem(preserveAspectRatio=true, extent={{-200,-150},{300,
            250}})),
    __Dymola_Commands(file="modelica://Annex60/Resources/Scripts/Dymola/Airflow/Multizone/Examples/CO2TransportStep.mos"
        "Simulate and plot"),
    experiment(
      StopTime=86400,
      Tolerance=1e-05),
