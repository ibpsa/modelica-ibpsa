...
model ClosedDoors "Model with three closed doors"
  ...
  Modelica.Blocks.Math.Gain Gain1(k=100) annotation (Placement(transformation(
          extent={{-28,40},{-8,60}})));
  Annex60.Fluid.MixingVolumes.MixingVolume volC(
    redeclare package Medium = Medium,
    V=2.5*5*5,
    nPorts=4,
    energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial,
  ...
  Annex60.Airflow.Multizone.DoorDiscretizedOperable dooBC(
    redeclare package Medium = Medium,
    LClo=20*1E-4,
    forceErrorControlOnFlow=true) "Discretized door"
...
equation
  ...
  connect(volA.ports[4], dooAB.port_a1) annotation (Line(
      ...
      color={0,127,255}));
  annotation (
experiment(Tolerance=1e-006, StartTime=0.0, StopTime=7200),
...
<p>
This model uses
<a href=\"modelica://Annex60.Media.Specialized.Air.PerfectGas\">
