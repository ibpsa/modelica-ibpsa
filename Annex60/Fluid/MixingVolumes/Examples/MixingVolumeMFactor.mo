...
model MixingVolumeMFactor
  "A check for verifying the implementation of the parameter mSenFactor"
  ...
  Annex60.Fluid.MixingVolumes.MixingVolume volMFactor(
    ...
    "Mixing volume using mSenFactor = 10"
  ...
  Annex60.Fluid.MixingVolumes.MixingVolume vol1(
    ...
    nPorts=2,
    energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial)
    "MixingVolume with V = 10 instead of mSenFactor = 10"
  ...
  Annex60.Fluid.Sources.MassFlowSource_T boundaryMFactor(
    ...
    nPorts=1,
    m_flow=1,
    X={0.02,0.98}) "Flow source for mixing volume using mSenFactor"
...
<p>This model contains two verifications for the implementation of <code>mSenFactor</code>:</p>
<ol>
<li>
The mixingVolume temperature <code>vol.T</code> should be constant. 
This is to check the correct implementation of the parameter <code>mSenFactor</code> for moist air media.
</li>
<li>
...
difference between using an <code>mSenFactor = 10</code> and multiplying volume by <i>10</i>.
</li>
</ol>
</html>", revisions="<html>
<ul>
