...
model RadiatorEN442_2 "Dynamic radiator for space heating"
  ...
  parameter SI.Temperature T_a_nominal
    "Water inlet temperature at nominal condition"
    annotation(Dialog(group="Nominal condition"));
  parameter SI.Temperature T_b_nominal
    "Water outlet temperature at nominal condition"
    annotation(Dialog(group="Nominal condition"));
  parameter SI.Temperature TAir_nominal = 293.15
    "Air temperature at nominal condition"
    annotation(Dialog(group="Nominal condition"));
  parameter SI.Temperature TRad_nominal = TAir_nominal
    "Radiative temperature at nominal condition"
    annotation(Dialog(group="Nominal condition"));

  parameter Real n = 1.24 "Exponent for heat transfer";
  parameter SI.Volume VWat = 5.8E-6*abs(Q_flow_nominal)
  ...
  parameter SI.Mass mDry = 0.0263*abs(Q_flow_nominal)
...
protected
   ...
   parameter SI.TemperatureDifference[nEle] dTCon_nominal(
   ...
   parameter SI.ThermalConductance UAEle(fixed=false, min=0,
   ...
   SI.TemperatureDifference dTCon[nEle] = heatPortCon.T .- vol.T
    "Temperature difference for convective heat transfer";
   SI.TemperatureDifference dTRad[nEle] = heatPortRad.T .- vol.T
