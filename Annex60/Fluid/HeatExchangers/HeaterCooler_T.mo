...
model TemperatureControlledHeatFlow "Prescribed heat flow boundary condition, depending on a set temperature TSet"
...
model HeaterCooler_T "Ideal heater or cooler with a prescribed outlet temperature"
  ...
  parameter Modelica.SIunits.HeatFlowRate Q_flow_maxHeat(min=0,max=Modelica.Constants.inf) = Modelica.Constants.inf
    "Maximum heat flow rate for heating (positive)";
  parameter Modelica.SIunits.HeatFlowRate Q_flow_maxCool(min=-Modelica.Constants.inf,max=0.0) = - Modelica.Constants.inf
    "Maximum heat flow rate for cooling (negative)";
  ...
  TemperatureControlledHeatFlow preHea(redeclare package Medium = Medium, Q_flow_maxHeat = Q_flow_maxHeat, Q_flow_maxCool = Q_flow_maxCool,
    m_flow = port_a.m_flow, h_outflow = inStream(port_a.h_outflow), Xi = inStream(port_a.Xi_outflow), p = port_a.p)
