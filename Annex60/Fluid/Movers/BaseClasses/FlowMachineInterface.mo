...
partial model FlowMachineInterface
  ...
  parameter Annex60.Fluid.Movers.BaseClasses.Characteristics.powerParameters power
  ...
  parameter Real powDer[size(power.V_flow,1)]=
   if use_powerCharacteristic then
     Annex60.Utilities.Math.Functions.splineDerivatives(
                   x=power.V_flow,
                   y=power.P,
                   ensureMonotonicity=Annex60.Utilities.Math.Functions.isMonotonic(x=power.P,
                                                                                     strict=false))
   else
...
initial algorithm
  ...
  assert(Annex60.Utilities.Math.Functions.isMonotonic(x=pressure.V_flow, strict=true) and
  pressure.V_flow[1] > -Modelica.Constants.eps,
  "The volume flow rate for the fan pressure rise must be a strictly increasing sequence
...
the simulation stops.");
  ...
  if (haveVMax and haveDPMax) or (nOri == 2) then  // ----- Curve 1
    ...
    preDer1:=Annex60.Utilities.Math.Functions.splineDerivatives(x=pCur1.V_flow, y=pCur1.dp);
...
<a href=\"modelica://Annex60.Fluids.Movers.BaseClasses.PrescribedFlowMachine\">PrescribedFlowMachine</a>.
</p>
<p>
