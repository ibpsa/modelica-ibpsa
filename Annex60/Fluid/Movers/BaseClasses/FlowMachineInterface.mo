  ...
  parameter Real powDer[size(_per_y.power.V_flow,1)]=
   if _per_y.use_powerCharacteristic then
     Annex60.Utilities.Math.Functions.splineDerivatives(
                   ...
                   y=_per_y.power.P,
                   ensureMonotonicity=Annex60.Utilities.Math.Functions.isMonotonic(x=_per_y.power.P,
                                                                                     strict=false))
   else
...
initial algorithm
  ...
  assert(Annex60.Utilities.Math.Functions.isMonotonic(x=_per_y.pressure.V_flow, strict=true) and
