...
block HotWaterTemperatureReset
  ...
  parameter SI.Temperature TSup_nominal "Supply temperature"
    annotation (Dialog(group="Nominal conditions"));
  parameter SI.Temperature TRet_nominal "Return temperature"
    annotation (Dialog(group="Nominal conditions"));
  parameter SI.Temperature TRoo_nominal = 293.15
    "Room temperature"
    annotation (Dialog(group="Nominal conditions"));
  parameter SI.Temperature TOut_nominal "Outside temperature"
    annotation (Dialog(group="Nominal conditions"));

  parameter Boolean use_TRoo_in = false
    "Get the room temperature set point from the input connector"
    annotation(Evaluate=true, HideResult=true);
  parameter SI.Temperature TRoo = 293.15
    "Fixed value of room temperature set point"
    annotation(Dialog(enable = not use_TRoo_in));
  parameter SI.TemperatureDifference dTOutHeaBal(displayUnit="K") = 8
...
protected
  ...
  SI.Temperature TOutOffSet
    "Effective outside temperature for heat transfer (takes into account room heat gains)";
  parameter SI.Temperature TOutOffSet_nominal =  TOut_nominal + dTOutHeaBal
