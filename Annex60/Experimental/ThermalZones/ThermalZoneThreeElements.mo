...
model ThermalZoneThreeElements
  ...
  BaseClasses.ExtMassVarRC floorRC(
    ...
    RExt=RGround,
    RExtRem=RGroundRem,
    CExt=CGround) if AGroundInd > 0 "RC-element for floor plate" annotation (
      Placement(transformation(
        extent={{10,-11},{-10,11}},
        rotation=90,
        origin={-12,-152})));
  Modelica.Thermal.HeatTransfer.Components.ConvectiveResistor convFloor if     AGroundInd > 0
    annotation (Placement(transformation(
        extent={{-10,10},{10,-10}},
        rotation=90,
        origin={-12,-116})));
  Modelica.Blocks.Sources.Constant alphaFloor(k=1/(AGroundInd*alphaGroundInd)) if  AGroundInd > 0
    annotation (Placement(transformation(
        extent={{-5,-5},{5,5}},
        rotation=180,
        origin={22,-116})));
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor resIntWallFloor(R=1/(min(
    ...
    "resistor between interior walls and floor" annotation (Placement(
        ...
        rotation=0,
        origin={204,-104})));
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a floor if         AGroundInd > 0
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor resFloorWin(R=1/(min(
        AWinInd, AGroundInd)*alphaRad)) if AWinInd > 0 and AGroundInd > 0
...
equation
  ...
  connect(convFloor.fluid, volAir.heatPort) annotation (Line(
      ...
      color={191,0,0},
      smooth=Smooth.None));
  connect(alphaFloor.y, convFloor.Rc) annotation (Line(
  ...
  connect(floorRC.port_b, floor) annotation (Line(
      ...
      color={191,0,0},
      smooth=Smooth.None));
  connect(resFloorWin.port_a, heatConWin.solid) annotation (Line(
  ...
  connect(intMassVarRC.port_a, resIntWallFloor.port_a) annotation (Line(points={
  ...
  connect(resFloorWin.port_b, resExtWallFloor.port_b) annotation (Line(points={{
          ...
          0}));
  connect(resExtWallFloor.port_a, heatConExt.solid) annotation (Line(points={{-144,
