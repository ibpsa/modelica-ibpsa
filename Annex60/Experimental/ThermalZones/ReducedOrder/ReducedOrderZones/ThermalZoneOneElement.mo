...
model ThermalZoneOneElement
  ...
  parameter Modelica.SIunits.CoefficientOfHeatTransfer alphaRad
    "Coefficient of heat transfer for linearized radiation exchange between
    walls"
    annotation(Dialog(group="Thermal zone"));
...
public
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a intGainsConv if
    ATot > 0 or VAir > 0
   "Auxilliary port for internal convective gains"
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a extWallIndoorSurface if
    indoorPortExtWalls
    "Auxilliary port at indoor surface of exterior walls"
    annotation (Placement(
...
equation
  ...
  connect(TMeanRadSensor.T, TMeanRad)
    annotation (Line(points={{176,128},{176,134},
