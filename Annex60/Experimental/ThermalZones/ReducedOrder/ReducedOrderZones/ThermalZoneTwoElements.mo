...
model ThermalZoneTwoElements
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resIntWallWin(
    G=min(AWin, AInt)*alphaRad) if  AWin > 0 and AInt > 0
...
equation
  ...
  connect(convIntWall.solid, intWallRC.port_a)
    annotation (Line(
    points={{148,-40},{182.4,-40}},
    color={191,0,0},
    smooth=Smooth.None));
  connect(intWallRC.port_a, resExtWallIntWall.port_b)
    annotation (Line(
    points={{182.4,-40},{168,-40},{168,-106},{158,-106}},
    color={191,0,0},
    smooth=Smooth.None));
  ...
  elseif AExt > 0 and AWin > 0 and AInt > 0 then
    connect(thermSplitterIntGains.signalOutput[3], intWallRC.port_a)
      annotation (Line(
      points={{190,86},{190,86},{190,86},{160,86},{160,-40},{182.4,-40}},
      color={191,0,0},
      smooth=Smooth.None));
    connect(thermSplitterSolRad.signalOutput[3], intWallRC.port_a)
      annotation (
      Line(
      points={{-136,146},{-58,146},{-58,96},{160,96},{160,-40},{182.4,-40}},
      color={191,0,0},
      smooth=Smooth.None));
  end if;
  connect(resIntWallWin.port_b, intWallRC.port_a)
    annotation (Line(
    points={{94,-108},{118,-108},{118,-88},{168,-88},{168,-40},{182.4,-40}},
    color={191,0,0},
    smooth=Smooth.None));
  ...
  connect(intWallRC.port_a, intWallIndoorSurface)
    annotation (Line(points={{182.4,-40},{168,-40},{168,-82},{-120,-82},{-120,
          -180}},
    color={191,0,0}));
