...
model ThermalZoneTwoElements
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a intWallIndoorSurface if
    indoorPortIntWalls
    "Auxilliary port at indoor surface of interior walls"
...
equation
  ...
  connect(alphaIntWall.y, convIntWall.Gc)
    annotation (Line(points={{138,-51.5},{138,-49.75},{138,-46}},
    color={0,0,127}));
  connect(intWallRC.port_a, intWallIndoorSurface)
    annotation (Line(points={{182.4,
    -36},{170,-36},{170,-78},{-120,-78},{-120,-170}},
    color={191,0,0}));
  connect(convIntWall.fluid, TIndAirSensor.port)
    annotation (Line(points={{128,
  ...
  thermal masses and outer walls. While outer walls contribute to heat transfer
  to the ambient, adiabatic conditions apply to internal masses. Internal wall
  element needs length of RC-chain <code>nInt</code>, vector of capacities
