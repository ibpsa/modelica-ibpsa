...
model A60NumEquations
  "Checking number of equations with A60 implementation"
  ...
  PipeHeatLossMod pipeHeatLossMod(nPorts=1,
    redeclare package Medium = Medium,
    diameter=0.05,
    length=100,
  ...
  parameter Types.ThermalResistanceLength R=1/(pipeHeatLossMod.lambdaI*2*
      Modelica.Constants.pi/Modelica.Math.log((pipeHeatLossMod.diameter/2 +
      pipeHeatLossMod.thicknessIns)/(pipeHeatLossMod.diameter/2)));
equation
  ...
  connect(pipeHeatLossMod.port_a, source.ports[1]) annotation (Line(points={{20,
          30},{-26,30},{-26,28},{-68,28}}, color={0,127,255}));
  connect(pipeHeatLossMod.ports_b[1], sink.ports[1]) annotation (Line(points={{40,
          30},{80,30},{80,28},{120,28}}, color={0,127,255}));
  connect(pipeHeatLossMod.heatPort, fixedTemperature.port) annotation (Line(
        points={{30,40},{28,40},{28,88},{0,88},{0,90}}, color={191,0,0}));
