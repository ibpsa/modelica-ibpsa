...
model SpatialDistributionOperator
  ...
  Annex60.Fluid.Sensors.MassFlowRate masFloA2( redeclare package Medium =
        Medium) "Mass flow rate sensor for the A60 temperature delay"
    annotation (Placement(transformation(extent={{68,96},{88,116}})));
...
equation
  ...
  connect(PAtm.y, sin2.p_in) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(senTemA60In1.port_b, PipeDelayMod.port_a) annotation (Line(points={{
          -36,-12},{-12,-12},{14,-12}}, color={0,127,255}));
  connect(PipeDelayMod.port_b, senTemA60Out1.port_a)
    annotation (Line(points={{34,-12},{42,-12},{50,-12}}, color={0,127,255}));
  connect(senTemA60In2.port_b, PipeDelay.port_a)
    annotation (Line(points={{-48,106},{2,106},{2,106}}, color={0,127,255}));
  connect(PipeDelay.port_b, senTemA60Out2.port_a)
    annotation (Line(points={{22,106},{38,106},{38,106}}, color={0,127,255}));
  ...
  connect(fixedTemperature1.port, PipeDelay.heatPort)
    annotation (Line(points={{-2,154},{12,154},{12,116}}, color={191,0,0}));
