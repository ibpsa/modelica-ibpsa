...
model SingleLayerLucent "single non-opaque layer"

extends IDEAS.Building.Elements.StateDouble;
extends Modelica.Blocks.Interfaces.BlockIcon;

  parameter Modelica.SIunits.Area A "surface area";
  parameter BWF.Bui.Mat mat "material";
...
protected
  ...
  Real Gr = if mat.gas then 9.81*beta*(mat.rho^2)*(mat.d^3)/(mat.mhu^2)*abs(dT) else 0
    "Grrashof number";
  Real Nu = if mat.gas then Commons.Math.MaxSmooth(1,0.0384*abs(Gr)^(0.37),0.01) else 1
    "Nusselt number";
