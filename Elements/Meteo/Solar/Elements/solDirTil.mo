within IDEAS.Elements.Meteo.Solar.Elements;
model solDirTil
...
algorithm
  if inc <= Modelica.Constants.small then
    solDirTil := IDEAS.Elements.Math.MaxSmooth(0,A*sim.solDirHor, delta=0.01);
  else
    solDirTil := IDEAS.Elements.Math.MaxSmooth(0,cos(angSol)*sim.solDirPer*A, delta=0.01);
  end if;
end solDirTil;
