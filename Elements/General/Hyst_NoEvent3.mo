...
block Hyst_NoEvent3 "Hysteresis without events, with Real in- and output"
  ...
  output Real error;
equation
  y =  Hysteresis_NoEvent(u, y, uLow, uHigh);
  error = if noEvent(u < uHigh and u > (uLow + 1.01*(uHigh-uLow)) and der(u) < 0 and y < 0.5) then 1.0 else 0.0;
  assert(error<0.5, "The Hyst_NoEvent did not operate correctly.  Try reducing the tolerance of the solver");
  annotation (
