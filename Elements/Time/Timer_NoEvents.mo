...
block Timer_NoEvents "Min-on OR min-off timer that does not generate events"
  ...
  */
  ...
  import IDEAS.Elements.Time.Elements.TimerType;
...
equation
  if timerType==TimerType.off then
    y = if noEvent(IDEAS.Elements.General.Hysteresis_NoEvent(cap.T, y, TStart + TStep/2, TEnd) > 0.5 or der(cap.T) <= 0 or cap.T < (TStart + Modelica.Constants.small)) then 1 else 0;
  else
    y = if noEvent(IDEAS.Elements.General.Hysteresis_NoEvent(cap.T, 1-y, TStart + TStep/2, TEnd) > 0.5 or der(cap.T) <= 0 or cap.T < (TStart + Modelica.Constants.small)) then 0 else 1;
  end if;
