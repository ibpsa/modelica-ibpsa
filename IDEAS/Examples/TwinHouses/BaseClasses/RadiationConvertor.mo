...
public
  Modelica.Blocks.Sources.RealExpression HDirSouth(y=(H_south - solDifHor/2 -
    ...
    annotation (Placement(transformation(extent={{-80,2},{0,24}})));
  Modelica.Blocks.Sources.RealExpression HDirEast(y=(H_east - solDifHor/2 -
        rhoGai[1].y/2)*IDEAS.Utilities.Math.Functions.inverseXRegularized(cos(
        incEast.incAng), 0.01))
  ...
  Modelica.Blocks.Sources.RealExpression HDirWest(y=(H_west - solDifHor/2 -
        rhoGai[3].y/2)*IDEAS.Utilities.Math.Functions.inverseXRegularized(cos(
        incWest.incAng), 0.01))
