...
// still not working as it should
  ...
  IDEAS.Climate.Time.Timer_NoEvents offTimer(duration=if minOffTime > 0 then
        minOffTime else 666, timerType=Commons.Time.Elements.TimerType.off)
    annotation (Placement(transformation(extent={{-44,28},{-24,48}})));
  IDEAS.Climate.Time.Timer_NoEvents onTimer(duration=if minOnTime > 0 then
        minOnTime else 666, timerType=Commons.Time.Elements.TimerType.on)
