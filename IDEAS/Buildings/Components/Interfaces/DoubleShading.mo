...
model DoubleShading "Two shading components in series"
  extends StateShading(final controlled = stateShading1.controlled or stateShading2.controlled);
  replaceable StateShading stateShading1
    constrainedby StateShading(azi=azi) "First shading device"
    annotation (Placement(transformation(extent={{-28,-18},{-18,2}})));
  replaceable StateShading stateShading2
    constrainedby StateShading(azi=azi) "Second shading device"
