...
// Maximum powers
parameter Modelica.SIunits.Power PMax = 4600*numPha;                          // Max. charging power [W], limited by grid connection
parameter Modelica.SIunits.Power PCharge = min(PMax,(EBat * 1000 * e_c));     // Max. charging power [W], limited by battery type
parameter Modelica.SIunits.Power PDischarge = min(PMax,(EBat * 1000 * e_d));  // Max. discharging power [W], limited by battery type

// Variables
Modelica.SIunits.Power P;        // Net power = difference Ppv and Phouseholds
