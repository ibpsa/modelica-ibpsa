...
parameter Boolean singlePhaseGrid=true
    "Is it a single phase equivalent grid, or 3 phase grid"
 annotation(choices(
...
replaceable parameter IDEAS.Electric.Data.Interfaces.Cable
                                           typHouBran=IDEAS.Electric.Data.Cables.PvcAl16()
...
protected
parameter Integer numPhaGriCon= if singlePhaseGrid then 1 else
                                                              (if connectedPhase==4 then 4 else 3);
...
parameter Real mulFacZ = if connectedPhase==0 then 1/3 else
    (if connectedPhase==4 then 1
    else 2);
