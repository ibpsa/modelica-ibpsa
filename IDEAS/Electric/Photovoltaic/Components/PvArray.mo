...
equation
  connect(incidenceAngles.angIncDir, refDir.angInc)
                                                   annotation (Line(
      points={{-18,36},{-8,36}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(incidenceAngles.angIncDif, refDif.angInc)
                                                   annotation (Line(
      points={{-18,32},{-14,32},{-14,12},{-8,12}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(incidenceAngles.angIncRef, refRef.angInc)
                                                   annotation (Line(
  ...
  connect(pvSerie.pinSer, p) annotation (Line(
      ...
      color={0,0,255},
      smooth=Smooth.None));
  connect(cos.y, incidenceAngles.angInc)
                                        annotation (Line(
  ...
  connect(radSol.angHou, cos2.u) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(cos2.y, incidenceAngles.angHou)
                                         annotation (Line(
