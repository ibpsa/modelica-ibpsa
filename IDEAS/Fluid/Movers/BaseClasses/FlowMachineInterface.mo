  ...
  parameter Real powDer[size(power.V_flow,1)]=
   if use_powerCharacteristic then
     IDEAS.Utilities.Math.Functions.splineDerivatives(
                   x=power.V_flow,
                   y=power.P,
                   ensureMonotonicity=IDEAS.Utilities.Math.Functions.isMonotonic(x=power.P,
                                                                                     strict=false))
   else
...
initial algorithm
  ...
  assert(IDEAS.Utilities.Math.Functions.isMonotonic(x=pressure.V_flow, strict=true) and
...
the simulation stops.");
  ...
  if (haveVMax and haveDPMax) or (nOri == 2) then  // ----- Curve 1
    ...
    preDer1:=IDEAS.Utilities.Math.Functions.splineDerivatives(x=pCur1.V_flow, y=pCur1.dp);
  ...
  elseif haveVMax or haveDPMax then  // ----- Curve 2
    ...
    pCur3.dp     := zeros(nOri + 2);
    preDer1:=zeros(nOri);
    preDer2:=IDEAS.Utilities.Math.Functions.splineDerivatives(x=pCur2.V_flow, y=pCur2.dp);
    preDer3:=zeros(nOri+2);
    ...
    preDer1:=zeros(nOri);
    preDer2:=zeros(nOri+1);
    preDer3:=IDEAS.Utilities.Math.Functions.splineDerivatives(x=pCur3.V_flow, y=pCur3.dp);
...
equation
  ...
  if use_powerCharacteristic then
    ...
    // a division by zero.
    eta = WFlo / IDEAS.Utilities.Math.Functions.smoothMax(x1=P, x2=1E-5, deltaX=1E-6);
    ...
    // a division by zero.
    P = WFlo / IDEAS.Utilities.Math.Functions.smoothMax(x1=eta, x2=1E-5, deltaX=1E-6);
...
<a href=\"modelica://IDEAS.Fluids.Movers.BaseClasses.PrescribedFlowMachine\">PrescribedFlowMachine</a>.
</p>
<p>
