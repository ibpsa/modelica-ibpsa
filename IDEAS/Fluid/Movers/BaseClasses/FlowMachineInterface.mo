...
partial model FlowMachineInterface
  ...
  extends IDEAS.Fluid.Movers.BaseClasses.PowerInterface(
  ...
  parameter Real powDer[size(_per_y.power.V_flow,1)]=
   if _per_y.use_powerCharacteristic then
     IDEAS.Utilities.Math.Functions.splineDerivatives(
                   ...
                   y=_per_y.power.P,
                   ensureMonotonicity=IDEAS.Utilities.Math.Functions.isMonotonic(x=_per_y.power.P,
                                                                                     strict=false))
   else
...
initial algorithm
  ...
  assert(IDEAS.Utilities.Math.Functions.isMonotonic(x=_per_y.pressure.V_flow, strict=true) and
...
the simulation stops.");
  ...
  if (haveVMax and haveDPMax) or (nOri == 2) then  // ----- Curve 1
    ...
    // used in equation 20 in IDEAS/Resources/Images/Fluid/Movers/UsersGuide/2013-IBPSA-Wetter.pdf
    // see function IDEAS.Fluid.Movers.BaseClasses.Characteristics.flowApproximationAtOrigin
    cBar[1] :=cha.pressure(
      per=pCur1,
      V_flow=0,
      r_N=delta,
 ...
 motDer :=if _per_y.use_powerCharacteristic then zeros(size(_per_y.motorEfficiency.V_flow,
    ...
    ensureMonotonicity=IDEAS.Utilities.Math.Functions.isMonotonic(x=_per_y.motorEfficiency.eta,
      strict=false));
  hydDer :=if _per_y.use_powerCharacteristic then zeros(size(_per_y.hydraulicEfficiency.V_flow,
    1)) elseif (size(_per_y.hydraulicEfficiency.V_flow, 1) == 1) then {0}
     else IDEAS.Utilities.Math.Functions.splineDerivatives(x=_per_y.hydraulicEfficiency.V_flow,
...
equation
  ...
  if _per_y.use_powerCharacteristic then
    ...
    // a division by zero.
    _eta = WFlo / IDEAS.Utilities.Math.Functions.smoothMax(x1=_PEle, x2=1E-5, deltaX=1E-6);
    ...
    // a division by zero.
    _PEle = WFlo / IDEAS.Utilities.Math.Functions.smoothMax(x1=eta, x2=1E-5, deltaX=1E-6);
...
<a href=\"modelica://IDEAS.Fluids.Movers.BaseClasses.FlowControlledMachine\">FlowControlledMachine</a>.
</p>
<p>
