...
model PrescribedOutletState
  ...
  extends IDEAS.Fluid.Interfaces.PartialTwoPortTransport(
    final dp_start=0,
    show_T=false,
    show_V_flow=false);
  extends IDEAS.Fluid.Interfaces.PrescribedOutletStateParameters(
...
equation
  ...
  if not restrictHeat and not restrictCool then
    ...
    if restrictHeat and restrictCool then
      ...
      dhSetAct = IDEAS.Utilities.Math.Functions.smoothLimit(
    ...
    elseif restrictHeat then
      ...
      dhSetAct = IDEAS.Utilities.Math.Functions.smoothMin(
        x1=hSet-inStream(port_a.h_outflow),
        x2=Q_flow_maxHeat / m_flow_limited,
        deltaX=deltah);
    else
      // Capacity limit for cooling only
      dhSetAct = IDEAS.Utilities.Math.Functions.smoothMax(
        x1=hSet-inStream(port_a.h_outflow),
...
See <a href=\"modelica://IDEAS.Fluid.HeatExchangers.HeaterCooler_T\">
IDEAS.Fluid.HeatExchangers.HeaterCooler_T</a>
