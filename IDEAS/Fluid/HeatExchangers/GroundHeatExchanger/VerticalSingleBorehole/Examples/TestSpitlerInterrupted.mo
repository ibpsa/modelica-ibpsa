within IDEAS.Fluid.HeatExchangers.GroundHeatExchanger.VerticalSingleBorehole.Examples;
model TestSpitlerInterrupted "validation spitler: Interrupted test"
  ...
  Modelica.Blocks.Sources.Pulse pulse(
    ...
    offset=590,
    startTime=9*3600,
    width=4) annotation (Placement(transformation(extent={{-94,26},{-74,46}})));

  parameter Thermal.Data.Interfaces.Medium medium=
...
equation
  ...
  connect(pulse.y, prescribedHeatFlow.Q_flow) annotation (Line(
      points={{-73,36},{-68,36},{-68,46},{-60,46}},
      color={0,0,127},
      smooth=Smooth.None));
  annotation (
    Diagram(graphics),
...
<p>This test is an interrupted test. A constant heat input is applied, but for a certain time period of the experiment, the heater is switched off.. All parameters were taken from the paper, except the thermal capacity of the sand. The thermal capacity of the sand was calculated the measurements of the temperature of the sand at certain positions.</p>
