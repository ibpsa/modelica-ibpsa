...
algorithm
  ...
  tmpSha := IBPSA.Utilities.Cryptographics.sha(strGen);
  for i in 1:i_strCoo loop
    tmpSha :=tmpSha + IBPSA.Utilities.Cryptographics.sha(strCoo[i]);
  end for;

  // Create a sha from tmpSha
  sha := IBPSA.Utilities.Cryptographics.sha(tmpSha);
