...
protected
  ...
  Fluid.MixingVolumes.MixingVolume vol_b(
    ...
    final V=if rho_default > 500 then VEqu/2 else VEqu/2/1000,
    final nPorts=2,
  ...
  MixingVolumes.MixingVolume vol_a(
    ...
    final energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial,
    final mSenFac=if rho_default > 500 then 1 else 10) if have_pipCap
    "Control volume connected to port_a. Represents equivalent pipe wall thermal capacity."
...
equation
  ...
  connect(senMasFlo.m_flow,timDel. m_flow) annotation (Line(
      points={{-30,-11},{-30,-50},{-12,-50}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(timDel.tauRev,heaLos_a. tau) annotation (Line(points={{11,-46},{14,
          -46},{14,50},{-44,50},{-44,10}},
                                      color={0,0,127}));
  connect(timDel.tau,heaLos_b. tau) annotation (Line(points={{11,-54},{30,-54},
          {30,16},{44,16},{44,10}}, color={0,0,127}));
  connect(heaLos_a.port_a,senMasFlo. port_a)
    annotation (Line(points={{-40,0},{-40,0}}, color={0,127,255}));
  connect(del.port_a,res. port_b)
    annotation (Line(points={{1.77636e-15,0},{-1.83187e-15,10}},
                                            color={0,127,255}));
  connect(senMasFlo.port_b,res. port_a)
    annotation (Line(points={{-20,0},{-12,0},{-12,40},{1.77636e-15,40},{
          1.77636e-15,30}},                    color={0,127,255}));
  connect(heaLos_b.port_a,del. port_b)
    annotation (Line(points={{40,0},{20,0},{20,-30},{0,-30},{0,-20},{
          -1.77636e-15,-20}},                color={0,127,255}));
  connect(heaLos_b.heatPort, heatPort) annotation (Line(points={{50,10},{50,86},
          {0,86},{0,100}}, color={191,0,0}));
  connect(heaLos_a.heatPort, heatPort) annotation (Line(points={{-50,10},{-50,
  ...
  connect(heaLos_b.port_b, noMixPip_b.port_a) annotation (Line(points={{60,0},{
          66,0},{66,-20},{70,-20}}, color={0,127,255}));
  connect(noMixPip_b.port_b, port_b) annotation (Line(points={{90,-20},{94,-20},
  ...
  connect(noMixPip_a.port_b, heaLos_a.port_b) annotation (Line(points={{-70,-20},
          {-66,-20},{-66,0},{-60,0}}, color={0,127,255}));
