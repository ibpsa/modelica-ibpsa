...
<p>The component calculates the delay time at both in/outlet ports of the pipe and therefore has two outlets. During forward flow, only the forward <a href=\"modelica://IBPSA/Fluid/PlugFlowPipes/BaseClasses/HeatLossPipeDelay\">HeatLossPipeDelay</a> component in <a href=\"modelica://IBPSA/Fluid/PlugFlowPipes/BaseClasses/PipeCore\">PipeCore</a> will be active and uses the forward output of PlugFlowTransportDelay. During reverse, the opposite is true and only the reverse output is used. </p>
<h4>Assumption</h4>
