...
protected
  ...
  parameter Real etaCarnot_nominal_internal(
    unit="1") = if use_eta_Carnot_nominal then
      ...
      COP_nominal / (TUse_nominal/(TCon_nominal+TAppCon_nominal - (TEva_nominal-TAppEva_nominal)))
  ...
  final parameter Modelica.SIunits.Temperature TUse_nominal=
   if COP_is_for_cooling then TEva_nominal-TAppEva_nominal else TCon_nominal+TAppCon_nominal
