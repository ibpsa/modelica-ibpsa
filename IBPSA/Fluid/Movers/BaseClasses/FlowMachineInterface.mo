...
model FlowMachineInterface
  ...
  parameter IBPSA.Fluid.Movers.BaseClasses.Types.PrescribedVariable preVar=
    IBPSA.Fluid.Movers.BaseClasses.Types.PrescribedVariable.Speed "Type of prescribed variable";
...
initial equation
  ...
  assert(IBPSA.Utilities.Math.Functions.isMonotonic(x=per.pressure.V_flow, strict=true) and
...
equation
  ...
  if (curve == 1) then
    if homotopyInitialization then
       V_flow*kRes + dp_internal = homotopy(actual=IBPSA.Fluid.Movers.BaseClasses.Characteristics.pressure(
                                                     V_flow=V_flow,
                                                     r_N=r_N,
                                                     dpMax=dpMax,
                                                     ...
                                                     d=preDer1,
                                                     per=pCur1),
                                           simplified=r_N * (IBPSA.Fluid.Movers.BaseClasses.Characteristics.pressure(
                                                     ...
                                                     d=preDer1,
                                                     per=pCur1)
                               +(V_flow-V_flow_nominal) * (IBPSA.Fluid.Movers.BaseClasses.Characteristics.pressure(
                                                     ...
                                                     d=preDer1,
                                                     per=pCur1)
                                       -IBPSA.Fluid.Movers.BaseClasses.Characteristics.pressure(V_flow=(1-delta)*V_flow_nominal,
                                                     r_N=1,
                                                     dpMax=dpMax,
                                                     V_flow_max=V_flow_max,
                                                     d=preDer1,
       ...
       V_flow*kRes + dp_internal= IBPSA.Fluid.Movers.BaseClasses.Characteristics.pressure(V_flow=V_flow,
                                               r_N=r_N,
                                               dpMax=dpMax,
                                                     V_flow_max=V_flow_max,
                                               d=preDer1,
  ...
  if per.etaHydMet==
       IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate then
    if homotopyInitialization then
      P_internal = homotopy(actual=IBPSA.Fluid.Movers.BaseClasses.Characteristics.power(per=per.power, V_flow=V_flow, r_N=r_N, d=powDer, delta=delta),
                      simplified=V_flow/V_flow_nominal*
                            IBPSA.Fluid.Movers.BaseClasses.Characteristics.power(per=per.power, V_flow=V_flow_nominal, r_N=1, d=powDer, delta=delta));
    else
      P_internal = (rho/rho_default)*IBPSA.Fluid.Movers.BaseClasses.Characteristics.power(per=per.power, V_flow=V_flow, r_N=r_N, d=powDer, delta=delta);
    end if;
  ...
  elseif per.etaHydMet==
    ...
    if homotopyInitialization then
      P_internal = homotopy(actual=IBPSA.Fluid.Movers.BaseClasses.Characteristics.power(per=powEu, V_flow=V_flow, r_N=r_N, d=powEuDer, delta=delta),
                      simplified=V_flow/V_flow_nominal*
                            IBPSA.Fluid.Movers.BaseClasses.Characteristics.power(per=powEu, V_flow=V_flow_nominal, r_N=1, d=powEuDer, delta=delta));
    else
      P_internal = (rho/rho_default)*IBPSA.Fluid.Movers.BaseClasses.Characteristics.power(per=powEu, V_flow=V_flow, r_N=r_N, d=powEuDer, delta=delta);
    end if;
    eta_internal = WFlo / IBPSA.Utilities.Math.Functions.smoothMax(
                            x1=P_internal, x2=deltaP, deltaX=deltaP/2);
  elseif per.etaHydMet == IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Efficiency_VolumeFlowRate then
    if homotopyInitialization then
      eta_internal = homotopy(actual=IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.efficiency,     V_flow=V_flow, d=etaDer, r_N=r_N, delta=delta),
                        simplified=IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.efficiency, V_flow=V_flow_max,   d=etaDer, r_N=r_N, delta=delta));
    else
      eta_internal = IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.efficiency, V_flow=V_flow, d=etaDer, r_N=r_N, delta=delta);
    end if;
  ...
  if per.etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_VolumeFlowRate then
    if homotopyInitialization then
      etaMot = homotopy(actual=IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency,     V_flow=V_flow, d=motDer, r_N=r_N, delta=delta),
                        simplified=IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency, V_flow=V_flow_max,   d=motDer, r_N=r_N, delta=delta));
    else
      etaMot = IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency,     V_flow=V_flow, d=motDer, r_N=r_N, delta=delta);
    end if;
  elseif per.etaMotMet==
       IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio then
    if homotopyInitialization then
      etaMot =homotopy(actual=IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
        per=per.motorEfficiency_yMot,
        y=yMot,
        d=motDer_yMot), simplified=IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
        per=per.motorEfficiency_yMot,
        y=1,
        d=motDer_yMot));
    else
      etaMot =IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
        per=per.motorEfficiency_yMot,
        y=yMot,
        d=motDer_yMot);
    end if;
  elseif per.etaMotMet==
       IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve then
      if homotopyInitialization then
        etaMot =homotopy(actual=IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
          per=per.motorEfficiency_yMot_generic,
          y=yMot,
          d=motDer_yMot_generic), simplified=IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
          per=per.motorEfficiency_yMot_generic,
          y=1,
...
<a href=\"modelica://IBPSA.Fluid.Movers.BaseClasses.PowerInterface\">
...
<code>etaMot = IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency, V_flow=V_flow, d=motDer, r_N=r_N, delta=1E-4)</code>
