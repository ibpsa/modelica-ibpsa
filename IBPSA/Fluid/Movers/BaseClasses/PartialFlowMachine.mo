...
partial model PartialFlowMachine
  ...
  parameter Modelica.SIunits.Time riseTime=30
    ...
    annotation(Dialog(tab="Dynamics", group="Filtered speed",enable=use_input_filter));
...
protected
  ...
  Modelica.Blocks.Sources.Constant[size(stageInputs, 1)] stageValues(
    final k=stageInputs) if
      inputType == IBPSA.Fluid.Types.InputType.Stages "Stage input values"
  ...
  Extractor extractor(final nin=size(stageInputs,1)) if
      inputType == IBPSA.Fluid.Types.InputType.Stages "Stage input extractor"
  ...
  Modelica.Blocks.Math.Gain gaiSpe(y(final unit="1")) if
    inputType == IBPSA.Fluid.Types.InputType.Continuous and
    speedIsInput
