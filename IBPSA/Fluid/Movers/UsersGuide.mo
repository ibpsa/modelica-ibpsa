...
<p>
All models have a parameter <code>use_input_filter</code>. This
parameter affects the fan output as follows:
</p>
<ol>
<li>
If <code>use_input_filter=false</code>, then the input signal <code>y</code> (or
...
<li>
If <code>use_input_filter=true</code>, which is the default,
...
<p>
The figure below shows for a fan with <code>use_input_filter=true</code>
...
of <code>use_input_filter=true</code> and <code>riseTime=30</code> seconds.
An exception are situations in which the fan or pump is operated at a fixed speed during
the whole simulation. In this case, set <code>use_input_filter=false</code>.
</p>
<p>
Note that if the fan is part of a closed loop control, then the filter affects
the transient response of the control.
When changing the value of <code>use_input_filter</code>, the control gains
...
and a fan or pump, configured with <code>use_input_filter=true</code> and <code>riseTime=30</code> seconds.
