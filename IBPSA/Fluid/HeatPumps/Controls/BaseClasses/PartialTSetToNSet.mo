...
partial model PartialTSetToYSet
  ...
  IBPSA.Utilities.Logical.SmoothSwitch swiNullHP "If HP is off, zero is passed"
  ...
  Modelica.Blocks.Interfaces.RealInput TAct(
    ...
    annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-116,-80})));
  IBPSA.Utilities.Logical.SmoothSwitch swiNullsecHeaGen if use_secHeaGen
    "If second heater is off, zero is passed" annotation (Placement(
        transformation(
        extent={{8,-8},{-8,8}},
        rotation=90,
        origin={12,-84})));
  IBPSA.Utilities.Math.MovingAverage movAve(final aveTime=movAveTime)
