...
partial model PartialSolarCollector "Partial model for solar collectors"
 extends IBPSA.Fluid.Interfaces.LumpedVolumeDeclarations;
  ...
  parameter Modelica.Units.SI.HeatCapacity CTot=
    if per.CTyp==IBPSA.Fluid.SolarCollectors.Types.HeatCapacity.TotalCapacity then
      per.C
    elseif per.CTyp==IBPSA.Fluid.SolarCollectors.Types.HeatCapacity.DryCapacity then
  ...
  parameter IBPSA.Fluid.SolarCollectors.Types.NumberSelection nColType=
  IBPSA.Fluid.SolarCollectors.Types.NumberSelection.Number
  ...
  parameter Integer nPanels= 0 "Desired number of panels in the simulation"
    annotation(Dialog(group="Area declarations", enable= (nColType == IBPSA.Fluid.SolarCollectors.Types.NumberSelection.Number)));

  parameter Modelica.Units.SI.Area totalArea=0
    "Total area of panels in the simulation" annotation (Dialog(group=
          "Area declarations", enable=(nColType == IBPSA.Fluid.SolarCollectors.Types.NumberSelection.Area)));

  parameter IBPSA.Fluid.SolarCollectors.Types.SystemConfiguration sysConfig=
  ...
  IBPSA.BoundaryConditions.SolarIrradiation.DiffusePerez HDifTilIso(
...
protected
  ...
  parameter Real nPanels_internal=
    if nColType == IBPSA.Fluid.SolarCollectors.Types.NumberSelection.Number then
      nPanels
    else
