...
model PVOpticalAbsRat
  ...
  Modelica.Units.SI.Angle incAngGro
    "Incidence angle for ground reflection";

  Modelica.Units.SI.Angle incAngDif
  ...
  Modelica.Units.SI.Angle refAngGro
...
equation
  ...
  refAng = if (incAng >= Modelica.Constants.eps and
               incAng <= Modelica.Constants.pi/2*0.999)
           then asin(sin(incAng)/refInd)
           else 0;
  ...
  refAngGro = if (incAngGro >= Modelica.Constants.eps and
                  incAngGro <= Modelica.Constants.pi/2*0.999)
              then asin(sin(incAngGro)/refInd)
              else 0;
  ...
  tau = if (incAng >= Modelica.Constants.eps and
            ...
            refAng >= Modelica.Constants.eps)
        then exp(-(glaExtCoe*glaThi/cos(refAng)))*
            (1- 0.5*((sin(refAng - incAng)^2)/(sin(refAng + incAng)^2) +
            (tan(refAng - incAng)^2)/(tan(refAng + incAng)^2)))
        else 0;
  ...
  tau_gro = if (incAngGro >= Modelica.Constants.eps and
                refAngGro >= Modelica.Constants.eps)
            then exp(-(glaExtCoe*glaThi/cos(refAngGro)))*
  ...
  tau_dif = if (incAngDif >= Modelica.Constants.eps and
                refAngDif >= Modelica.Constants.eps)
            then exp(-(glaExtCoe*glaThi/cos(refAngDif)))*
  ...
  R_b = if ((zen >= Modelica.Constants.pi/2*0.999) or (cos(incAng)> cos(zen)*4))
        then 4
...
<p>
This is an optical model to calculate the absorption ratio of a PV cell based
on the radiation.
