...
<p>The package <code>IBPSA.Utilities.IO.RESTClient</code> consists of models that send/receive information to/from an external socket server in run time. Those models allow for example to sample the simulation data as the inputs for external conntrol deployment, or overwrite the control signals. </p>
<h4>Usage</h4>
...
<p>Next, to create an sampler for providing simulation data to external software, drag as many instances of <a href=\"modelica://IBPSA.Utilities.IO.RESTClient.Sampler\">/IBPSA.Utilities.IO.RESTClient.Sampler </a>and specify the host and TCP ip port number for the external socker server. </p>
<p>Likewise, one can also create an overwritten block using <a href=\"modelica://IBPSA.Utilities.IO.RESTClient.OverWritten\">IBPSA.Utilities.IO.RESTClient.OverWritten</a>. </p>
