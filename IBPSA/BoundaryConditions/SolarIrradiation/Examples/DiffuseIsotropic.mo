  ...
  IBPSA.BoundaryConditions.WeatherData.Bus weaBus "Weather data bus"
    annotation (Placement(
        transformation(extent={{-11,-1},{9,21}}),iconTransformation(extent={{1,
            -1},{2,-2}})));
  IBPSA.BoundaryConditions.SolarIrradiation.DiffuseIsotropic
        HDifRoo(til=IBPSA.Types.Tilt.Ceiling,
                rho=rho) "Diffuse irradiation on roof"
    annotation (Placement(transformation(extent={{40,0},{60,20}})));
  IBPSA.BoundaryConditions.SolarIrradiation.DiffuseIsotropic
        HDifFlo(til=IBPSA.Types.Tilt.Floor,
...
__Dymola_Commands(file="modelica://IBPSA/Resources/Scripts/Dymola/BoundaryConditions/SolarIrradiation/Examples/DiffuseIsotropic.mos"
        "Simulate and plot"));
