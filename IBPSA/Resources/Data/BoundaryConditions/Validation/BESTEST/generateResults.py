...
def _organize_cases(mat_dir):
    ...
    if len(CASES) == len(matFiles):
        ...
        raise ValueError(
            "*** There is failed simulation, no result file was found. Check the simulations.")
    return caseList
...
def _CleanTimeSeries(time, val, nPoi):
    ...
    if len(timen) != nPoi:
        raise ValueError(
            "Error: In _CleanTimeSeries, length and number of results points do not match.")
    return timen, valn
        ...
        for day in caseDays:
            if day['days'] in 'yearly':
                ...
                if not ressH:
                    ...
                    # manually update integrated values for 'integrated' variables for
                    # subhourly results
                    if 'horizontal_radiation' in ressH['json']:
                        ...
                        outDir[case]['subhourly_results'][day['days']
                                                          ]['integrated_' + ressH['json']] = sHRlist
    return outDir
...
if __name__ == '__main__':
    ...
    for Subcase in Subcases:
        if Subcase in 'Iso':
            ...
            CaseDictIsoHor['reVals'] = RemoveString(
                CaseDictIsoHor['reVals'], 'weaBusTDryBulTDewPoiOpa')
            resFinIsoHor = WeatherJson(resForm, Matfd, CaseDictIsoHor)
            ...
            resFinPerHor = WeatherJson(resForm, Matfd, CaseDictPerHor)
